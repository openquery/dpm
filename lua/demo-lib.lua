...
function new_client(c)
    ...
    dpml.register_callbacks(c, { [myp.MYC_SENT_CMD] = new_command,
                                 [myp.MY_CLOSING]   = client_closing,
                            })
...
function new_command(cmd_pkt, cid)
    ...
    if (cmd_pkt:argument() == "HELLO") then
        dpml.send_resultset(client, { 
                        fields = { 
                             { type = myp.MYSQL_TYPE_STRING, name = "salutations", },
                             { type = myp.MYSQL_TYPE_STRING, name = "number", },
                        },
                        rows = { { "hi", "1", }, { "hello", "2" }, 
                                 { "konnichiwa", "3" }, }
                        })
